...
Running this is exactly the same as the [previous example](../d-multi-containers-automated-setup/readme.md). The differences are only in the implementation. Here the image is built with the ssh stack included using a Dockerfile that is invoked via docker compose.
